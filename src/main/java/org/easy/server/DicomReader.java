...
public class DicomReader {
    ...
    /**
     * DicomReader
     * @description 构建dicom文件流
     * @param file 一个普通的 dcm 文件
     * @return 无
     * @author zoudaifa
     * @date 2019/3/23 20:17
     * @version 1.0.0
     */
    public DicomReader(File file) throws IOException {
        ...
        try {
            ...
            this.fmi = dis.readFileMetaInformation() != null ?
                    dis.readFileMetaInformation() :
                    attr.createFileMetaInformation(UID.ImplicitVRLittleEndian);

        } catch (IOException e) {
            LOG.error(e.getMessage());
            // what's the point of keeping this file? throw
            throw e;
        } finally {
            try {
                if (dis != null)
    ...
    public String toString() {
        ...
        return String.format("[Patient] => Patient ID: %s, Patient Name: %s, Patient Sex: %s, Patient Age: %s, Patient Birthday: %s \n"
                        + "----------------------------------------------------------------------------------------------------------------------\n"
                ...
                getPatientID(), getPatientName(), getPatientSex(), getPatientAge(), getPatientBirthDay(), getAccessionNumber(),
                getAdditionalPatientHistory(), getAdmittingDiagnosesDescription(), getReferringPhysicianName(), getSOPInstanceUID(),
                getStudyDateTime(), getStudyID(), getStudyInstanceUID(), getStudyPriorityID(), getStudyStatusID(), getBodyPartExamined(),
                getLaterality(), getOperatorsName(), getPatientPosition(), getProtocolName(), getSeriesDateTime(), getSeriesDescription(),
                getSeriesInstanceUID(), getSeriesNumber(), getConversionType(), getInstitutionalDepartmentName(), getDeviceSerialNumber(),
                getInstitutionAddress(), getInstitutionName(), getManufacturer(), getManufacturerModelName(), getModality(),
                getSoftwareVersion(), getStationName(), getAcquisitionDateTime(), getContentDateTime(), getExposureTime(),
                getImageOrientation(), getImagePosition(), getImageType(), getInstanceNumber(), getKvp(), getMediaStorageSopInstanceUID(),
                getTransferSyntaxUID(), getPatientOrientation(), getPixelSpacing(), getSliceLocation(), getSliceThickness(),
