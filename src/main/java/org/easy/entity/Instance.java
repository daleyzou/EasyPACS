 ...
 *可以是图像或SR文档
 * 在这里，我们只使用图像，所以我们将实例作为图像保存在服务器级别。
 * images
 */
@Entity
    ...
    public String toString() {
        return String
                .format("Instance[pkTBLInstanceID=%d, sopInstanceUID=%s, sopClassUID=%s, instanceNumber=%d, patientOrientation=%s, mediaStorageSopInstanceUID=%s, acquisitionDateTime=%s, imageType=%s, pixelSpacing=%f, imageOrientation=%s, imagePosition=%s, sliceThickness=%f, sliceLocation=%f, windowCenter=%s, windowWidth=%s, xrayTubeCurrent=%d, exposureTime=%d, kvp=%s, contentDateTime=%s, createdDate=%s, modifiedDate=%s]",
                        pkTBLInstanceID, sopInstanceUID, sopClassUID, instanceNumber, patientOrientation, mediaStorageSopInstanceUID,
                        acquisitionDateTime, imageType, pixelSpacing, imageOrientation, imagePosition, sliceThickness, sliceLocation,
                        windowCenter, windowWidth, xrayTubeCurrent, exposureTime, kvp, contentDateTime, createdDate, modifiedDate);
    }
